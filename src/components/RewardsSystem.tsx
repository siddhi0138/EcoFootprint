        ...
        <Card className="bg-white/80 backdrop-blur-sm border-green-200 shadow-lg dark:bg-gray-900/80 dark:border-green-700">
          <CardHeader>
            <CardTitle className="flex items-center justify-between text-green-700 dark:text-green-400">
              ...
              <Badge variant="outline" className="bg-green-50 text-green-600 border-green-200 dark:bg-green-900/50 dark:text-green-400 dark:border-green-700">
          ...
          <CardContent className="space-y-4">
            {dailyChallenges.map((challenge, index) => (
              <div key={index} className={`rounded-lg p-4 transition-all duration-200 ${
                challenge.completed ? 'bg-green-50 border border-green-200 dark:bg-green-800 dark:border-green-700' : 'bg-gray-50 border border-gray-200 dark:bg-gray-800 dark:border-gray-700'
              }`}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    {challenge.completed && <CheckCircle className="w-4 h-4 text-green-600 dark:text-green-400" />}
                    <span className={`font-medium ${challenge.completed ? 'text-green-800 dark:text-green-400' : 'text-gray-700 dark:text-gray-300'}`}>
                      {challenge.task}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Badge variant="outline" className={`text-xs ${
                      challenge.completed ? 'bg-green-100 text-green-700 border-green-300 dark:bg-green-700 dark:text-green-400 dark:border-green-600' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'
                    }`}>
                      {challenge.category}
                    </Badge>
                    <Badge variant={challenge.completed ? "default" : "secondary"} className="bg-emerald-100 text-emerald-700 dark:bg-emerald-700 dark:text-emerald-300">
                      +{challenge.points}
                    </Badge>
                  </div>
                </div>
            ...
            {achievements.map((achievement) => (
              <div key={achievement.id} className={`p-4 rounded-lg border transition-all duration-200 ${
                achievement.unlocked 
                  ? 'bg-emerald-50 border-emerald-200 shadow-sm dark:bg-emerald-900 dark:border-emerald-700' 
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className={`font-semibold ${
                        achievement.unlocked ? 'text-green-800 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'
                      }`}>
                        {achievement.name}
                      </h3>
                    ...
                    <p className={`text-sm ${
                      achievement.unlocked ? 'text-green-600 dark:text-green-300' : 'text-gray-400 dark:text-gray-500'
                    }`}>
                  ...
                  <Badge variant={achievement.unlocked ? "default" : "secondary"} className="bg-emerald-100 text-emerald-700 dark:bg-emerald-700 dark:text-emerald-300">
                    +{achievement.points}
                  </Badge>
                </div>
              </div>
        ...
        <CardContent>
          {userStats.totalPoints < 150 ? (
            <div className="text-center py-12">
              <Gift className="w-20 h-20 text-gray-300 dark:text-gray-600 mx-auto mb-6" />
              <h3 className="text-xl font-semibold text-gray-600 dark:text-gray-300 mb-3">Start earning rewards!</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-4">Complete challenges and track your sustainability journey to unlock amazing rewards.</p>
              <div className="bg-green-50 rounded-lg p-4 border border-green-200 dark:bg-green-900/50 dark:border-green-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {rewards.map((reward) => (
                <div key={reward.id} className={`p-6 rounded-xl border transition-all duration-200 hover:shadow-lg ${
                  reward.available 
                    ? 'bg-white border-green-200 hover:border-green-300 dark:bg-gray-800 dark:border-green-700 hover:dark:border-green-600' 
                    : 'bg-gray-50 border-gray-200 opacity-75 dark:bg-gray-900 dark:border-gray-700'
                }`}>
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-green-800 dark:text-green-400">{reward.name}</h3>
                      <Badge variant="outline" className="text-xs mt-1 dark:text-gray-400">
                        {reward.category}
                      </Badge>
                    </div>
                  </div>
                  
                  <p className="text-sm text-green-600 dark:text-green-400 mb-3">{reward.description}</p>
                  
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-xs text-green-500 font-medium dark:text-green-400">
                      {reward.savings}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="font-semibold text-green-800 dark:text-green-400">{reward.cost}</span>
                    </div>
                    <Button 
                      size="sm" 
